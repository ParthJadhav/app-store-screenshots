  ...
  return (
    <div className="space-y-2 rounded border bg-background/70 p-2.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Field label="Opacity"><Input aria-label="Layer opacity" type="number" min={0} max={100} value={Math.round((layer.opacity ?? 1) * 100)} onChange={(event) => onPatch({ opacity: Number(event.target.value) / 100 })} /></Field>
        <Field label="Rotation"><Input aria-label="Layer rotation" type="number" min={-180} max={180} value={Math.round(layer.transform.rotation || 0)} onChange={(event) => onPatch({ transform: { ...layer.transform, rotation: Number(event.target.value) } } as Partial<VisualLayer>)} /></Field>
      </div>
      {layer.kind === "text" && <TextDetails layer={layer} locale={locale} onPatch={onPatch} />}
      {layer.kind === "image" && <><ScreenshotPicker label="Layer image" value={layer.src} locale={locale} onChange={(src) => onPatch({ src } as Partial<VisualLayer>)} /><div className="grid grid-cols-2 gap-2"><Field label="Fit"><Select value={layer.fit || "cover"} onValueChange={(fit) => onPatch({ fit } as Partial<VisualLayer>)}><SelectTrigger aria-label="Image fit"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="cover">Cover</SelectItem><SelectItem value="contain">Contain</SelectItem><SelectItem value="fill">Fill</SelectItem></SelectContent></Select></Field><Field label="Radius"><Input aria-label="Image corner radius" type="number" value={layer.borderRadius || 0} onChange={(event) => onPatch({ borderRadius: Number(event.target.value) } as Partial<VisualLayer>)} /></Field></div></>}
      {layer.kind === "shape" && <div className="grid grid-cols-2 gap-2"><Field label="Shape"><Select value={layer.shape} onValueChange={(shape) => onPatch({ shape } as Partial<VisualLayer>)}><SelectTrigger aria-label="Shape type"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="rectangle">Rectangle</SelectItem><SelectItem value="ellipse">Ellipse</SelectItem><SelectItem value="line">Line</SelectItem></SelectContent></Select></Field><Field label="Fill"><Input aria-label="Shape fill" type="color" value={layer.fill || "#5B7CFA"} onChange={(event) => onPatch({ fill: event.target.value } as Partial<VisualLayer>)} /></Field></div>}
      <div className="grid grid-cols-2 gap-1">
...
function TextDetails({ layer, locale, onPatch }: { layer: TextLayer; locale: string; onPatch: (patch: Partial<VisualLayer>) => void }) {
  return <><Field label="Text"><Textarea aria-label="Layer text" rows={2} value={layer.text[locale] ?? pickText(layer.text, locale)} onChange={(event) => onPatch({ text: { ...layer.text, [locale]: event.target.value } } as Partial<VisualLayer>)} /></Field><div className="grid grid-cols-2 gap-2"><Field label="Size"><Input aria-label="Text size" type="number" value={layer.fontSize || 72} onChange={(event) => onPatch({ fontSize: Number(event.target.value) } as Partial<VisualLayer>)} /></Field><Field label="Color"><Input aria-label="Text color" type="color" value={layer.color || "#171717"} onChange={(event) => onPatch({ color: event.target.value } as Partial<VisualLayer>)} /></Field></div></>;
}
