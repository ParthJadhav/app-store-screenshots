...
**The exporter intermittently drops the screenshot inside the device frame.** You get a perfect headline, background and frame with a blank device screen, no error, and a correct on-canvas preview. Observed 18 Aug 2026 across repeated exports of one deck: a different slide failed each run, and the *same* slide dropped at 4K while rendering correctly at 1080p in the same run. Root cause not established.
...
**Then open the sheets and look.** The count is triage, not proof. Two automated detectors were tried during that session — mean brightness and colourfulness — and both gave false readings in both directions, one of them passing a broken file. A missing screenshot is unmissable beside its siblings on a contact sheet and easy to miss when opening files one at a time.
